...
coords.smooth.roc <- function(smooth.roc,
  ...
  if (is.character(x)) {
    ...
    rownames(res) <- rep("best", nrow(res))
...
coords.roc <- function(roc,
      ...
      res <- data.frame(
    	...
    	for (i in seq_along(x)) {
    		...
    		if (sp %in% roc$sp) {
    			idx <- match(sp, roc$sp)
    			browser()
    			res[i, 1] <- roc$thresholds[idx]
    			res[i, 2:3] <- c(roc$sp[idx], roc$se[idx])
    		}
  ...
  if (any(! ret %in% colnames(res))) {
  	...
  	res <- roc.utils.calc.coords(substr.percent, res$threshold, res$specificity, res$specificity, ncases, ncontrols, best.weights)
  }
